...
class MP3TaggerApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.views_dir: str = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data\\views")
        ...
        self.section_configs_dir: str = os.path.join(os.path.dirname(__file__), "data", "section_configs")
    ...
    def set_filter_string(self, *args) -> None:
        ...
        for section in self.sections:
            section_filter = self.get_filter_from_section("", section, control_toggle_pressed=False)
            if section_filter:
    ...
    def launch_section_editor(self) -> None:
        ...
        from mp3tagger.classes.section_editor_window import SectionEditorWindow
        
   
        editor = SectionEditorWindow(
            parent=self.root,
    ...
    def save_sections_data(self, new_sections: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            config_file = os.path.join(os.path.dirname(__file__), "data", "sections_config.json")
            ...
            messagebox.showinfo("Success", "Sections configuration saved successfully!")
            
        except Exception as e:
            logger.error(f"Failed to save sections data: {e}")
            messagebox.showerror("Error", f"Failed to save sections data: {str(e)}")
    ...
    def build_section(self, window, section, row, column, toggle_command, command):
        ...
        scale = self.build_scale(window, section['label'], '      *', section['group_control'], toggle_command=toggle_command, command=command)
        ...
        for s in section['scales']:
            row = row + 1
            scale = self.build_scale(window, section['label'], param=s[0], value=s[1], toggle_command=toggle_command, command=command)
    ...
    def get_filter_from_section(self, current_filter: str, section: Dict[str, Any], control_toggle_pressed: bool = False) -> str:
        ...
        if label_text == "%_folderpath%" and off_filters:
            new_filters.append(f"(NOT %_folderpath% MATCHES {('|'.join(off_filters))})")
        else:
