...
class SectionEditorWindow:
    def __init__(self, parent: tk.Tk, sections_data: List[Dict[str, Any]], save_callback) -> None:
        ...
        self.style.configure('Dark.TLabelframe', background=self.bg_color, foreground=self.fg_color)
        self.style.configure('Dark.TLabelframe.Label', background=self.bg_color, foreground=self.fg_color)
        self.style.configure('Dark.TButton', background=self.button_bg, foreground=self.fg_color)
        self.style.configure('Dark.TLabel', background=self.bg_color, foreground=self.fg_color)
        self.style.configure('Dark.TEntry', fieldbackground=self.entry_bg, foreground=self.fg_color)
        ...
        self.button_frame = ttk.Frame(self.scrollable_frame, style='Dark.TFrame')
        self.button_frame.pack(fill=tk.X, pady=(0, 10))
        
        # Add buttons using themed buttons
        self._create_button("Add Section", self.add_section).pack(side=tk.LEFT, padx=5)
        self._create_button("Save Changes", self.save_changes).pack(side=tk.LEFT, padx=5)
        self._create_button("Cancel", self.window.destroy).pack(side=tk.LEFT, padx=5)
    ...
    def add_section(self, section_data: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Label entry
        self._create_label(header_frame, "Section Label:").pack(side=tk.LEFT, padx=(0, 5))
    ...
    def delete_section(self, section_frame: tk.Frame) -> None:
        ...
        self.section_frames = [s for s in self.section_frames if s['frame'] != section_frame]
